...
- `src/infrastructure/ui/card_image_cache.py`: supporto mazzo napoletano — parametro `deck_type: str = "french"` a `CardImageCache.__init__()`; nuovi metodi `_load_source_napoletane()` e `get_back_bitmap()`; costanti di mapping seq napoletano (`_NAPOL_RANK_POS`, `_NAPOL_SUIT_OFFSET`, `_NAPOL_SEQ_TO_NOME`, `_NAPOL_SEME_OVERRIDE`). (card-assets-integration v4.3.0)
- `src/application/board_state.py`: campo `deck_type: str = "french"` a `BoardState` per trasportare il tipo di mazzo dall'application layer alla presentation senza accesso diretto alle settings. (card-assets-integration v4.3.0)
- `tests/unit/test_card_image_cache.py`: 16 nuovi test per path fix, routing napoletano, mapping sequenze, `get_back_bitmap`, `invalidate_size_cache` back-cache. (card-assets-integration v4.3.0)
- `tests/unit/test_card_renderer.py`: 3 nuovi test `TestBackBitmapRendering` per `back_bitmap` face-down, procedural fallback e ignore su face-up. (card-assets-integration v4.3.0)
- `tests/unit/presentation/test_options_dialog_tabs.py`: smoke test per notebook opzioni, label delle tab, mappa focus e `visual_theme_radio`. (options-tabs-layout v4.4.0)
- `tests/unit/presentation/test_options_formatter_help.py`: test unit per l’help aggiornato del dialogo opzioni wx. (options-tabs-layout v4.4.0)

### Fixed
- `src/infrastructure/audio/sound_mixer.py`: corretta l’incoerenza percettiva del volume sugli effetti sonori; i one-shot con panning usano ora una pan-law a potenza costante, così il volume percepito resta stabile tra eventi centrati e laterali invece di risultare più alto in alcune posizioni del tavolo.
- `src/infrastructure/config/audio_config_loader.py`, `src/infrastructure/audio/audio_manager.py`, `config/audio_config.json`: eliminata una regressione che poteva lasciare l’app completamente muta quando `audio_config.json` veniva salvato o modificato con sezioni `event_sounds` / `enabled_events` vuote; il loader ripristina ora default centralizzati e il manager usa fallback interni coerenti, mantenendo operativi musica ed effetti in menu e gameplay.
- `src/infrastructure/ui/options_dialog.py`, `src/infrastructure/audio/audio_manager.py`, `src/infrastructure/audio/sound_mixer.py`, `config/audio_config.json`: completata l’integrazione dei controlli `Volume Musica` e `Volume Effetti Sonori`; gli slider annunciano ora nome controllo + percentuale via TTS al focus e durante la modifica, il mixer applica correttamente il gain del bus anche ai one-shot con panning, il salvataggio audio preserva l’intera configurazione JSON (`event_sounds`, `enabled_events`, preload) e il volume musica governa centralmente i bus di sottofondo mentre il volume effetti governa i bus `gameplay` e `ui` in menu e gameplay.
- `src/infrastructure/ui/card_image_cache.py`: corretto il mapping dei rank francesi provenienti dal dominio (`Asso`, `Jack`, `Regina`, `Re`) verso i filename numerici degli asset (`1`, `11`, `12`, `13`); il renderer usa ora l'immagine corretta anche per assi e figure del mazzo francese invece di cercare path errati o ricadere su fallback incoerenti.
- `acs_wx.py` e `src/application/game_engine.py`: ripristinato il servizio di vocalizzazione durante il gameplay riusando lo stesso `screen_reader` inizializzato dal bootstrap wx anche nel `GameEngine`; evitata la creazione di un secondo stack TTS divergente che poteva lasciare il gameplay silenzioso pur con NVDA disponibile nel menu/app shell.
- `acs_wx.py`: bootstrap diagnostico anticipato per i build cx_Freeze; l'avvio frozen scrive sempre `startup_diagnostic.log` nella cartella dell'exe e registra anche le eccezioni precoci prima del logging applicativo standard. (cx-freeze-frozen-runtime v4.5.1)
- `src/infrastructure/config/runtime_root.py`, `src/infrastructure/config/audio_config_loader.py`, `src/infrastructure/config/scoring_config_loader.py`, `src/infrastructure/logging/categorized_logger.py`, `src/infrastructure/audio/audio_manager.py`: path runtime riallineati alla cartella dell'eseguibile frozen invece che al current working directory; log, config e sound pack vengono ora risolti correttamente anche da shortcut o da directory esterna. (cx-freeze-frozen-runtime v4.5.1)
- `src/infrastructure/accessibility/tts_provider.py` e `src/infrastructure/audio/audio_manager.py`: hardening dei sottosistemi opzionali; fallback TTS a provider dummy e audio no-op in caso di init fallita, senza bloccare l'avvio dell'app. (cx-freeze-frozen-runtime v4.5.1)
- `setup.py`: aggiunto `solitario-diag.exe` per smoke test console del runtime frozen e pulizia robusta della cartella `dist/solitario-classico` in ambienti OneDrive/Windows prima del rebuild. (cx-freeze-frozen-runtime v4.5.1)
- `src/infrastructure/ui/card_image_cache.py`: **BUG — path errato `"carte francesi"` (spazio) corretto in `"carte_francesi"` (underscore)**; la cache non riusciva a caricare nessuna carta francese perché la directory con spazio nel nome non esisteva nel filesystem. (card-assets-integration v4.3.0)
- `src/application/options_controller.py`: `_save_snapshot()` e `_restore_snapshot()` includono ora `draw_count`; annullare le modifiche dal dialogo opzioni ripristina correttamente anche il numero di carte pescate. (options-tabs-layout v4.4.0)
...
### Changed
- `src/infrastructure/ui/options_dialog.py` e `src/infrastructure/audio/audio_manager.py`: il tab `Audio e Accessibilità` espone ora due slider dedicati per `Volume Musica` e `Volume Effetti Sonori`; il controllo musica governa il sottofondo (`music` + `ambient`), mentre gli effetti aggiornano in modo coerente i bus `gameplay` e `ui`, con anteprima live durante la modifica, persistenza su `Salva` e rollback completo su `Annulla`/`ESC`.
- `src/infrastructure/ui/options_dialog.py`: layout completamente riorganizzato con `wx.Notebook` e quattro tab (`Generale`, `Gameplay`, `Audio e Accessibilità`, `Visuale`); i pulsanti `Salva` e `Annulla` restano esterni alle pagine, il focus iniziale e il cambio tab guidano NVDA verso il primo controllo logico della scheda attiva. (options-tabs-layout v4.4.0)
- `src/infrastructure/ui/options_dialog.py`: il tema visivo usa ora `visual_theme_radio` (`wx.RadioBox`) al posto della `Choice`, in coerenza con gli altri gruppi di opzioni e con una lettura più stabile da screen reader. (options-tabs-layout v4.4.0)
- `src/presentation/options_formatter.py`: `format_help_text()` descrive la navigazione reale del dialogo wx (`TAB`, frecce tra schede, `SPAZIO`, `INVIO`, `ESC`) invece dei vecchi comandi della finestra opzioni virtuale. (options-tabs-layout v4.4.0)
...
- `src/infrastructure/ui/card_renderer.py`: `draw_card()` accetta nuovo parametro `back_bitmap: object | None = None`; se fornito e la carta è coperta, usa `_draw_face_image()` con il bitmap del dorso invece del rendering procedurale. (card-assets-integration v4.3.0)
...
- `src/infrastructure/ui/gameplay_panel.py`: `_get_image_cache()` accetta `deck_type: str = "french"` e reinizializza la cache quando il tipo di mazzo cambia; `_on_paint()` propaga `state.deck_type` alla cache e ora richiede il `back_bitmap` per ogni carta coperta. `_paint_visual_background()` usa la stessa cache deck-type-aware. (card-assets-integration v4.3.0)
- `src/application/gameplay_controller.py`: `_build_board_state()` legge `self.engine.settings.deck_type` e popola `BoardState.deck_type`; fallback silenzioso a `"french"` se l'attributo è assente. (card-assets-integration v4.3.0)
